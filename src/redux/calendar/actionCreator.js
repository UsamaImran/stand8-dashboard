...
import { getRecruiterCalenderDataApi, getSalesCalenderDataApi } from './api';
import { CALENDER_DATA_TYPE } from '../../constants';
import { getCompanyCalendarHistory } from '../companyOverview/api';
...
const calendarGetData = data => {
  return async dispatch => {
    try {
      const { email, startDate, endDate, type } = data;
      dispatch(calendarReadBegin());
      if (type === CALENDER_DATA_TYPE.SALES) {
        const res = await getSalesCalenderDataApi({ email, startDate, endDate });
        dispatch(calendarReadSuccess(res.data));
      } else if (type === CALENDER_DATA_TYPE.RECRUITER) {
        const res = await getRecruiterCalenderDataApi({ email, startDate, endDate });
        dispatch(calendarReadSuccess(res.data));
      } else if (type === CALENDER_DATA_TYPE.COMPANY) {
        const res = await getCompanyCalendarHistory({ startDate, endDate });
        dispatch(calendarReadSuccess(res.data));
      }
    } catch (err) {

...
const getDirectionIcon = direction => {
  return direction === VOIP_CALL_DIRECTION.INBOUND.value ? (
    <FeatherIcon icon={VOIP_CALL_DIRECTION.INBOUND.icon} size={18} />
  ) : (
    <FeatherIcon icon={VOIP_CALL_DIRECTION.OUTBOUND.icon} size={18} />
  );
};

const getRoundedOffFormatValues = value => {
  return formatCurrenyValue(value).split('.')[0];
};

export { textRefactor, chartLinearGradient, customTooltips, getDirectionIcon, getRoundedOffFormatValues };

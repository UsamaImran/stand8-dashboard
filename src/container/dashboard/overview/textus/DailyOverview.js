...
import { textUsOverviewGetData } from '../../../../redux/textUs/actionCreator';
...
const DailyOverview = () => {
  ...
  const { rtl, textUsOverviewData } = useSelector(state => {
    return {
      ...
      textUsOverviewData: state.textUsReducer.textUsOverviewData,
    };
  });

  useEffect(() => {
    if (textUsOverviewGetData) {
      dispatch(textUsOverviewGetData());
    }
  }, [dispatch]);

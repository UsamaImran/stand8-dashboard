...
import { getContractorsEnding } from '../../../../redux/recruiterProfile/actionCreator';
...
const ContractorsEndingTable = ({ user }) => {
  ...
  const { contractorsEnding } = useSelector(state => {
    return {
      contractorsEnding: state.recruiterProfile.contractorsEnding,
    };
  });
  ...
  if (contractorsEnding) {
    contractorsEnding.map((ce, index) => {
      ...
      return dataSource.push({
        ...
        spread: formatCurrenyValue(ce.spread),
      });
    });
  }

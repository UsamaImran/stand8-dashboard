...
import { getWeeklySalesTarget } from '../../../../redux/recruiterProfile/actionCreator';
...
const SalesTarget = ({ user }) => {
  ...
  const { weeklySalesTarget, isLoading } = useSelector(state => {
    return {
      weeklySalesTarget: state.recruiterProfile.weeklySalesTarget,
  ...
  return (
    <SalesTargetWrap>
      <Cards title="President’s Club Goals ">
        {isLoading ? (
          <div
            className="sd-spin"

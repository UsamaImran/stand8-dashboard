...
import { PerformanceChartWrapper } from '../../style';
import { ChartjsAreaChart } from '../../../../components/charts/chartjs';
import { customTooltips } from '../../../../components/utilities/utilities';
import { getSpreadHistory } from '../../../../redux/recruiterProfile/actionCreator';
import { getQuotaValue } from '../../../../utility/sales-recruiterUtils';
import { formatCurrenyValue } from '../../../../utility/utility';

const SpreadHistoryChart = ({ user }) => {
  const { email, startDate } = user;
  const dispatch = useDispatch();
  const { spreadHistory, spreadHistoryLoading } = useSelector(state => {
    return {
      spreadHistory: state.recruiterProfile.spreadHistory,
      spreadHistoryLoading: state.recruiterProfile.spreadHistoryLoading,
    };
  });

  useEffect(() => {
    if (getSpreadHistory && email) {
      dispatch(getSpreadHistory({ email, directPlacement: false }));
    }
  }, [dispatch, email]);
  ...
  const onChangePlacementType = checked => {
    dispatch(getSpreadHistory({ email, directPlacement: checked }));
  };
